"""Sunbeam — a warm, unified Rich-based logger for Snakemake.

Everything the user sees lives inside ONE persistent Live-rendered frame:

    ┌─ SUNBEAM ─ snakemake* · editor · shell ────────────── host · py · clock ─┐
    │                                                                          │
    │   ☀  rnaseq-quant  workflow                     elapsed 04:12 · eta 02:38│
    │      ~/projects/rnaseq-quant/Snakefile                                   │
    │                                                                          │
    │   Total  Completed  Running  Queued  Failed  Cores                       │
    │    47      28         5       13      1     24/32                        │
    │   ████████████████░░░░░░░░░░░░  28/47  eta 02:38                         │
    │                                                                          │
    │   Throughput  6.8/min  ▁▂▃▄▅▆▇█▇▆                      peak 9.2/min      │
    │                                                                          │
    │ ── ACTIVE JOBS ──────────────   ── RULES ──────────────                   │
    │  ◐ #23 salmon_quant  1:42        salmon_quant  ████░░░  5/8               │
    │  ◐ #24 salmon_quant  1:28        fastqc_trim   ██████░  6/8 ·1✗          │
    │                                                                          │
    │                                  ── RECENT EVENTS ─────                  │
    │                                   14:32 ▸ job #27 started salmon_quant   │
    │                                   14:32 ✓ job #22 finished in 2m 14s     │
    │                                                                          │
    │ ── LAST SHELL ──────────────────────────────                              │
    │  salmon quant -i resources/salmon_index …                                │
    │                                                                          │
    ├─ logger sunbeam · theme monokai · cwd … ──────── q quit · v verbose · / ─┤
    └──────────────────────────────────────────────────────────────────────────┘

Handlers mutate shared state (active jobs, rule counters, event ring, etc.)
and call ``_refresh()`` — the Live display redraws the whole frame.

When ``Live`` is not running yet (e.g. workflow banner before the first
PROGRESS event), events fall back to ``console.print`` above where the frame
will later appear.
"""
...
# ────────────────────────────── palette ──────────────────────────────
# Warm "sunbeam" palette. All colors are truecolor hex so they degrade
# gracefully on 256-color terms (Rich handles the downsampling).
